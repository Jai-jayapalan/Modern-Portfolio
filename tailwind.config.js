...
const addVariablesForColors = ({ addBase, theme }) => {
  let allColors = theme("colors"); // Use theme("colors") directly
  let newVars = {};
...
const matchUtils = ({ matchUtilities, theme }) => {
  matchUtilities(
    {
      "bg-grid": (value) => ({
    ...
    { values: theme("backgroundColor"), type: "color" } // Remove flattenColorPalette
  );
};
